...
public sealed class SendCreditosSindicadosUseCase
{
    private readonly IEntitySource<CreditoSindicado> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCreditosSindicadosUseCase> _logger;

    public SendCreditosSindicadosUseCase(
        IEntitySource<CreditoSindicado> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCreditosSindicadosUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.CreditosSindicados.ToString();
            ...
            _logger.Info($"SendCreditosSindicados using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCreditosSindicados failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
