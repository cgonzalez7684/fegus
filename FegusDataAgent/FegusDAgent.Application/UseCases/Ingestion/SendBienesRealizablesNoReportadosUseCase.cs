...
public sealed class SendBienesRealizablesNoReportadosUseCase
{
    private readonly IEntitySource<BienesRealizablesNoReportados> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendBienesRealizablesNoReportadosUseCase> _logger;

    public SendBienesRealizablesNoReportadosUseCase(
        IEntitySource<BienesRealizablesNoReportados> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendBienesRealizablesNoReportadosUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.BienesRealizablesNoReportados.ToString();
            ...
            _logger.Info($"SendBienesRealizablesNoReportados using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendBienesRealizablesNoReportados failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
