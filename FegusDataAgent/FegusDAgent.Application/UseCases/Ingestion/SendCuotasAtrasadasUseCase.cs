...
public sealed class SendCuotasAtrasadasUseCase
{
    private readonly IEntitySource<CuotaAtrasada> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCuotasAtrasadasUseCase> _logger;

    public SendCuotasAtrasadasUseCase(
        IEntitySource<CuotaAtrasada> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCuotasAtrasadasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.CuotasAtrasadas.ToString();
            ...
            _logger.Info($"SendCuotasAtrasadas using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCuotasAtrasadas failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
