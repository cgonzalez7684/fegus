...
public sealed class SendCodeudoresUseCase
{
    private readonly IEntitySource<Codeudor> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCodeudoresUseCase> _logger;

    public SendCodeudoresUseCase(
        IEntitySource<Codeudor> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCodeudoresUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.Codeudores.ToString();
            ...
            _logger.Info($"SendCodeudores using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCodeudores failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
