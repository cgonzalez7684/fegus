...
public sealed class SendGarantiasMobiliariasUseCase
{
    private readonly IEntitySource<GarantiaMobiliaria> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendGarantiasMobiliariasUseCase> _logger;

    public SendGarantiasMobiliariasUseCase(
        IEntitySource<GarantiaMobiliaria> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendGarantiasMobiliariasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.GarantiasMobiliarias.ToString();
            ...
            _logger.Info($"SendGarantiasMobiliarias using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendGarantiasMobiliarias failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
