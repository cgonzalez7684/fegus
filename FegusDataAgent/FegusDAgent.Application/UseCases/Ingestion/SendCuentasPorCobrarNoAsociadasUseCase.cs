...
public sealed class SendCuentasPorCobrarNoAsociadasUseCase
{
    private readonly IEntitySource<CuentaPorCobrarNoAsociada> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCuentasPorCobrarNoAsociadasUseCase> _logger;

    public SendCuentasPorCobrarNoAsociadasUseCase(
        IEntitySource<CuentaPorCobrarNoAsociada> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCuentasPorCobrarNoAsociadasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.CuentasPorCobrarNoAsociadas.ToString();
            ...
            _logger.Info($"SendCuentasPorCobrarNoAsociadas using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCuentasPorCobrarNoAsociadas failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
