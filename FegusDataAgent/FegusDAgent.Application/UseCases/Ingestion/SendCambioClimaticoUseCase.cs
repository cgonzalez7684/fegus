...
public sealed class SendCambioClimaticoUseCase
{
    private readonly IEntitySource<CambioClimatico> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCambioClimaticoUseCase> _logger;

    public SendCambioClimaticoUseCase(
        IEntitySource<CambioClimatico> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCambioClimaticoUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.CambioClimatico.ToString();
            ...
            _logger.Info($"SendCambioClimatico using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCambioClimatico failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
