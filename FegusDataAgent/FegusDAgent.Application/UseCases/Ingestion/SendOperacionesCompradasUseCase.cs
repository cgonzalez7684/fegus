...
public sealed class SendOperacionesCompradasUseCase
{
    private readonly IEntitySource<OperacionComprada> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendOperacionesCompradasUseCase> _logger;

    public SendOperacionesCompradasUseCase(
        IEntitySource<OperacionComprada> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendOperacionesCompradasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.OperacionesCompradas.ToString();
            ...
            _logger.Info($"SendOperacionesCompradas using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendOperacionesCompradas failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
