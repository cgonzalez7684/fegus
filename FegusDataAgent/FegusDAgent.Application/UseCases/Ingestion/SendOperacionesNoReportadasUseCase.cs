...
public sealed class SendOperacionesNoReportadasUseCase
{
    private readonly IEntitySource<OperacionNoReportada> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendOperacionesNoReportadasUseCase> _logger;

    public SendOperacionesNoReportadasUseCase(
        IEntitySource<OperacionNoReportada> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendOperacionesNoReportadasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.OperacionesNoReportadas.ToString();
            ...
            _logger.Info($"SendOperacionesNoReportadas using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendOperacionesNoReportadas failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
