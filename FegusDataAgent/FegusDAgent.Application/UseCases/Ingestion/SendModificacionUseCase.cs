...
public sealed class SendModificacionUseCase
{
    private readonly IEntitySource<Modificacion> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendModificacionUseCase> _logger;

    public SendModificacionUseCase(
        IEntitySource<Modificacion> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendModificacionUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.Modificacion.ToString();
            ...
            _logger.Info($"SendModificacion using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendModificacion failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
