...
public sealed class SendCuentasXCobrarUseCase
{
    private readonly IEntitySource<CuentaPorCobrar> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendCuentasXCobrarUseCase> _logger;

    public SendCuentasXCobrarUseCase(
        IEntitySource<CuentaPorCobrar> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendCuentasXCobrarUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.CuentasXCobrar.ToString();
            ...
            _logger.Info($"SendCuentasXCobrar using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendCuentasXCobrar failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
