...
public sealed class SendFideicomisoUseCase
{
    private readonly IEntitySource<Fideicomiso> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendFideicomisoUseCase> _logger;

    public SendFideicomisoUseCase(
        IEntitySource<Fideicomiso> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendFideicomisoUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.Fideicomiso.ToString();
        ...
        {
            _logger.Error($"SendFideicomiso failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
