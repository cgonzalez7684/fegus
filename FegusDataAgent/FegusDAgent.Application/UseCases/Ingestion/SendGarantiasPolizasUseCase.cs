...
public sealed class SendGarantiasPolizasUseCase
{
    private readonly IEntitySource<GarantiaPoliza> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendGarantiasPolizasUseCase> _logger;

    public SendGarantiasPolizasUseCase(
        IEntitySource<GarantiaPoliza> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendGarantiasPolizasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.GarantiasPolizas.ToString();
            ...
            _logger.Info($"SendGarantiasPolizas using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendGarantiasPolizas failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
