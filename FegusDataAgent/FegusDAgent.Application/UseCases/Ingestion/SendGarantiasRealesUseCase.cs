...
public sealed class SendGarantiasRealesUseCase
{
    private readonly IEntitySource<GarantiaReal> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendGarantiasRealesUseCase> _logger;

    public SendGarantiasRealesUseCase(
        IEntitySource<GarantiaReal> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendGarantiasRealesUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.GarantiasReales.ToString();
            ...
            _logger.Info($"SendGarantiasReales using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendGarantiasReales failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
