...
public sealed class SendOrigenRecursosUseCase
{
    private readonly IEntitySource<OrigenRecursos> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendOrigenRecursosUseCase> _logger;

    public SendOrigenRecursosUseCase(
        IEntitySource<OrigenRecursos> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendOrigenRecursosUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.OrigenRecursos.ToString();
            ...
            _logger.Info($"SendOrigenRecursos using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendOrigenRecursos failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
