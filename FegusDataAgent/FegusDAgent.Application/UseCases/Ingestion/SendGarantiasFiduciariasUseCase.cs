...
public sealed class SendGarantiasFiduciariasUseCase
{
    private readonly IEntitySource<GarantiaFiduciaria> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendGarantiasFiduciariasUseCase> _logger;

    public SendGarantiasFiduciariasUseCase(
        IEntitySource<GarantiaFiduciaria> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendGarantiasFiduciariasUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.GarantiasFiduciarias.ToString();
            ...
            _logger.Info($"SendGarantiasFiduciarias using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendGarantiasFiduciarias failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
