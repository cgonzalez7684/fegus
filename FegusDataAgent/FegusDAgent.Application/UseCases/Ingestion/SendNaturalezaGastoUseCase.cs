...
public sealed class SendNaturalezaGastoUseCase
{
    private readonly IEntitySource<NaturalezaGasto> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendNaturalezaGastoUseCase> _logger;

    public SendNaturalezaGastoUseCase(
        IEntitySource<NaturalezaGasto> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendNaturalezaGastoUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.NaturalezaGasto.ToString();
            ...
            _logger.Info($"SendNaturalezaGasto using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendNaturalezaGasto failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
