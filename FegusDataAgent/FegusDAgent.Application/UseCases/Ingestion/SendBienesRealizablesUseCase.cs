...
public sealed class SendBienesRealizablesUseCase
{
    private readonly IEntitySource<BienesRealizables> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendBienesRealizablesUseCase> _logger;

    public SendBienesRealizablesUseCase(
        IEntitySource<BienesRealizables> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendBienesRealizablesUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.BienesRealizables.ToString();
            ...
            _logger.Info($"SendBienesRealizables using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendBienesRealizables failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
