...
public sealed class SendActividadEconomicaUseCase
{
    private readonly IEntitySource<ActividadEconomica> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendActividadEconomicaUseCase> _logger;

    public SendActividadEconomicaUseCase(
        IEntitySource<ActividadEconomica> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendActividadEconomicaUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.ActividadEconomica.ToString();
            ...
            _logger.Info($"SendActividadEconomica using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad}.");
        ...
        {
            _logger.Error($"SendActividadEconomica failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
