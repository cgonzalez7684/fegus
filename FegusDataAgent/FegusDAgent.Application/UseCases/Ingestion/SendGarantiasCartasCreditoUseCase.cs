...
public sealed class SendGarantiasCartasCreditoUseCase
{
    private readonly IEntitySource<GarantiaCartaCredito> _source;
    private readonly IIngestionSessionClient _sessionClient;
    private readonly IIngestionStreamSender _streamSender;
    private readonly IEventLogger<SendGarantiasCartasCreditoUseCase> _logger;

    public SendGarantiasCartasCreditoUseCase(
        IEntitySource<GarantiaCartaCredito> source,
        IIngestionSessionClient sessionClient,
        IIngestionStreamSender streamSender,
        IEventLogger<SendGarantiasCartasCreditoUseCase> logger)
    {
        _source = source;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var dataset = DataSetNameIngestion.GarantiasCartasCredito.ToString();
            ...
            _logger.Info($"SendGarantiasCartasCredito using sessionId={session.SessionId} state={session.SessionStateCode} for idLoad={box.IdLoad} for idLoadLocal={box.IdLoadLocal} lastSequence={lastSequence}.");
        ...
        {
            _logger.Error($"SendGarantiasCartasCredito failed for idLoadLocal={box.IdLoadLocal}.", ex);
            throw;
        }
    }
}
