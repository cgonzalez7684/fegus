...
public sealed class CuentaPorCobrarSource : IEntitySource<CuentaPorCobrar>
{
    ...
    public async IAsyncEnumerable<SourceRecord<CuentaPorCobrar>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<CuentaPorCobrar>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_cuentasxcobrar_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<CuentaPorCobrar>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
