...
public sealed class GarantiaMobiliariaSource : IEntitySource<GarantiaMobiliaria>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaMobiliaria>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaMobiliaria>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiasmobiliarias_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaMobiliaria>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
