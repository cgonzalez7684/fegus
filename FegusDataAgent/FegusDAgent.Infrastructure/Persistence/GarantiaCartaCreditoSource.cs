...
public sealed class GarantiaCartaCreditoSource : IEntitySource<GarantiaCartaCredito>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaCartaCredito>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaCartaCredito>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiascartascredito_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaCartaCredito>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
