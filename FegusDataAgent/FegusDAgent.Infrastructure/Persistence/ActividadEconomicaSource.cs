...
public sealed class ActividadEconomicaSource : IEntitySource<ActividadEconomica>
{
    private readonly IDbConnectionFactory _dbConnectionFactory;
    private readonly IEventLogger<ActividadEconomicaSource> _logger;

    public ActividadEconomicaSource(
        IDbConnectionFactory dbConnectionFactory,
        IEventLogger<ActividadEconomicaSource> logger)
    {
        _dbConnectionFactory = dbConnectionFactory;
        _logger = logger;
    }

    public async IAsyncEnumerable<SourceRecord<ActividadEconomica>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Error($"Failed to stream ActividadEconomica for idLoadLocal={idLoadLocal}.", ex);
                throw;
            }
    ...
    private async IAsyncEnumerable<SourceRecord<ActividadEconomica>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_actividadeconomica_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<ActividadEconomica>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }

    private static ActividadEconomica MapRow(NpgsqlDataReader reader) =>
        ActividadEconomica.Create(
            idLoadLocal:                  GetInt64(reader,   "id_load_local"),
            idOperacionCredito:           GetString(reader,  "idoperacioncredito"),
            tipoActividadEconomica:       GetString(reader,  "tipoactividadeconomica"),
            porcentajeActividadEconomica: GetDecimal(reader, "porcentajeactividadeconomica"));
