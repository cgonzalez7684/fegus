...
public sealed class FideicomisoSource : IEntitySource<Fideicomiso>
{
    ...
    public async IAsyncEnumerable<SourceRecord<Fideicomiso>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<Fideicomiso>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_fideicomiso_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<Fideicomiso>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
