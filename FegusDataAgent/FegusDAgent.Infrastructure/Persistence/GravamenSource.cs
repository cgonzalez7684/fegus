...
public sealed class GravamenSource : IEntitySource<Gravamen>
{
    ...
    public async IAsyncEnumerable<SourceRecord<Gravamen>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<Gravamen>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_gravamenes_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<Gravamen>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
