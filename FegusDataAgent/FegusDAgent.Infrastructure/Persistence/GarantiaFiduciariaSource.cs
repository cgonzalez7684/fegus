...
public sealed class GarantiaFiduciariaSource : IEntitySource<GarantiaFiduciaria>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaFiduciaria>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaFiduciaria>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiasfiduciarias_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaFiduciaria>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
