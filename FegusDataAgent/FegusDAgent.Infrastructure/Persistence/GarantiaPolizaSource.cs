...
public sealed class GarantiaPolizaSource : IEntitySource<GarantiaPoliza>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaPoliza>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaPoliza>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiaspolizas_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaPoliza>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
