...
public sealed class BienesRealizablesNoReportadosSource : IEntitySource<BienesRealizablesNoReportados>
{
    private readonly IDbConnectionFactory _dbConnectionFactory;
    private readonly IEventLogger<BienesRealizablesNoReportadosSource> _logger;

    public BienesRealizablesNoReportadosSource(
        IDbConnectionFactory dbConnectionFactory,
        IEventLogger<BienesRealizablesNoReportadosSource> logger)
    {
        _dbConnectionFactory = dbConnectionFactory;
        _logger = logger;
    }

    public async IAsyncEnumerable<SourceRecord<BienesRealizablesNoReportados>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Error($"Failed to stream BienesRealizablesNoReportados for idLoadLocal={idLoadLocal}.", ex);
                throw;
            }
    ...
    private async IAsyncEnumerable<SourceRecord<BienesRealizablesNoReportados>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_bienesrealizablesnoreportados_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<BienesRealizablesNoReportados>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }

    private static BienesRealizablesNoReportados MapRow(NpgsqlDataReader reader) =>
        BienesRealizablesNoReportados.Create(
            ...
            tipoMotivoBienRealizableNoReportado: GetString(reader,  "tipomotivobienrealizablenoreportado"),
            ultimoValorContable:                 GetDecimal(reader, "ultimovalorcontable"),
            valorRecuperadoNeto:                 GetDecimal(reader, "valorrecuperadoneto"),
            idOperacionCreditoFinanciamiento:    GetString(reader,  "idoperacioncreditofinanciamiento"));
