...
public sealed class GarantiaFacturaCedidaSource : IEntitySource<GarantiaFacturaCedida>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaFacturaCedida>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaFacturaCedida>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiasfacturascedidas_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaFacturaCedida>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
