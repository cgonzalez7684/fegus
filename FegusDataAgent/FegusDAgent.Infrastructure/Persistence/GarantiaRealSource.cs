...
public sealed class GarantiaRealSource : IEntitySource<GarantiaReal>
{
    ...
    public async IAsyncEnumerable<SourceRecord<GarantiaReal>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<GarantiaReal>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_garantiasreales_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<GarantiaReal>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
