...
public sealed class OperacionCompradaSource : IEntitySource<OperacionComprada>
{
    private readonly IDbConnectionFactory _dbConnectionFactory;
    private readonly IEventLogger<OperacionCompradaSource> _logger;

    public OperacionCompradaSource(
        IDbConnectionFactory dbConnectionFactory,
        IEventLogger<OperacionCompradaSource> logger)
    {
        _dbConnectionFactory = dbConnectionFactory;
        _logger = logger;
    }

    public async IAsyncEnumerable<SourceRecord<OperacionComprada>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Error($"Failed to stream OperacionesCompradas for idLoadLocal={idLoadLocal}.", ex);
                throw;
            }
    ...
    private async IAsyncEnumerable<SourceRecord<OperacionComprada>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_operacionescompradas_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<OperacionComprada>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }

    private static OperacionComprada MapRow(NpgsqlDataReader reader) =>
        OperacionComprada.Create(
            idLoadLocal:                 GetInt64(reader,    "id_load_local"),
            idOperacionCredito:          GetString(reader,   "idoperacioncredito"),
            tipoPersonaEntidadOperacion: GetString(reader,   "tipopersonaentidadoperacion"),
            identidadOperacionComprada:  GetString(reader,   "identidadoperacioncomprada"),
            tipoPersonaDeudor:           GetString(reader,   "tipopersonadeudor"),
            idDeudorComprada:            GetString(reader,   "iddeudorcomprada"),
            idOperacionCreditoComprada:  GetString(reader,   "idoperacioncreditocomprada"),
            fechaDesembolsoDeudor:       GetDateTime(reader, "fechadesembolsodeudor"));
