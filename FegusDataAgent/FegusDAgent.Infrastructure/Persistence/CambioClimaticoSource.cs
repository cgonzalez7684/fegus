...
public sealed class CambioClimaticoSource : IEntitySource<CambioClimatico>
{
    ...
    public async IAsyncEnumerable<SourceRecord<CambioClimatico>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<CambioClimatico>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_cambioclimatico_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<CambioClimatico>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
