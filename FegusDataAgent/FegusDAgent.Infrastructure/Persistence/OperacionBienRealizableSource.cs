...
public sealed class OperacionBienRealizableSource : IEntitySource<OperacionBienRealizable>
{
    private readonly IDbConnectionFactory _dbConnectionFactory;
    private readonly IEventLogger<OperacionBienRealizableSource> _logger;

    public OperacionBienRealizableSource(
        IDbConnectionFactory dbConnectionFactory,
        IEventLogger<OperacionBienRealizableSource> logger)
    {
        _dbConnectionFactory = dbConnectionFactory;
        _logger = logger;
    }

    public async IAsyncEnumerable<SourceRecord<OperacionBienRealizable>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Error($"Failed to stream OperacionesBienesRealizables for idLoadLocal={idLoadLocal}.", ex);
                throw;
            }
    ...
    private async IAsyncEnumerable<SourceRecord<OperacionBienRealizable>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_operacionesbienesrealizables_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<OperacionBienRealizable>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }

    private static OperacionBienRealizable MapRow(NpgsqlDataReader reader) =>
        OperacionBienRealizable.Create(
