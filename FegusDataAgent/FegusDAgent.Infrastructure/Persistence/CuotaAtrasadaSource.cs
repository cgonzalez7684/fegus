...
public sealed class CuotaAtrasadaSource : IEntitySource<CuotaAtrasada>
{
    private readonly IDbConnectionFactory _dbConnectionFactory;
    private readonly IEventLogger<CuotaAtrasadaSource> _logger;

    public CuotaAtrasadaSource(
        IDbConnectionFactory dbConnectionFactory,
        IEventLogger<CuotaAtrasadaSource> logger)
    {
        _dbConnectionFactory = dbConnectionFactory;
        _logger = logger;
    }

    public async IAsyncEnumerable<SourceRecord<CuotaAtrasada>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                _logger.Error($"Failed to stream CuotasAtrasadas for idLoadLocal={idLoadLocal}.", ex);
                throw;
            }
    ...
    private async IAsyncEnumerable<SourceRecord<CuotaAtrasada>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_cuotasatrasadas_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<CuotaAtrasada>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }

    private static CuotaAtrasada MapRow(NpgsqlDataReader reader) =>
        CuotaAtrasada.Create(
            ...
            tipoCuota:           GetString(reader,  "tipocuota"),
            numeroCuotaAtrasada: GetInt32(reader,   "numerocuotaatrasada"),
            diasAtraso:          GetInt32(reader,   "diasatraso"),
            montoCuotaAtrasada:  GetDecimal(reader, "montocuotaatrasada"));
