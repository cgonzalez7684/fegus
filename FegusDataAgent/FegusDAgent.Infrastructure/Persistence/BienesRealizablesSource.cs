...
public sealed class BienesRealizablesSource : IEntitySource<BienesRealizables>
{
    ...
    public async IAsyncEnumerable<SourceRecord<BienesRealizables>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<BienesRealizables>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_bienesrealizables_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<BienesRealizables>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
