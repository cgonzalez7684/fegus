...
public sealed class ModificacionSource : IEntitySource<Modificacion>
{
    ...
    public async IAsyncEnumerable<SourceRecord<Modificacion>> GetDataStreamAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
    ...
    private async IAsyncEnumerable<SourceRecord<Modificacion>> ReadFromDatabaseAsync(
        int? idCliente,
        long? idLoadLocal,
        long lastSeq,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            """
            SELECT *
            FROM feguslocal.obtener_modificacion_lista(@id_load_local, @last_seq)
            """;
        ...
        while (await reader.ReadAsync(cancellationToken))
            yield return new SourceRecord<Modificacion>(reader.GetInt64(reader.GetOrdinal("seq")), MapRow(reader));
    }
